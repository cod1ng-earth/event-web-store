...
module Checkout
    exposing
        ...
        , stockDecoder
        , toCheckoutContextEncoder
        , toProductEncoder
        , toCartEncoder
        , toPositionEncoder
        , toChangeProductQuantityEncoder
        , toOrderCartEncoder
        , toOrderCartResonseEncoder
        , toStockEncoder
        )
...
@docs CheckoutContextMsg, CheckoutContext, Product, Cart, Position, ChangeProductQuantity, OrderCart, OrderCartResonse, Stock


# Decoder

@docs checkoutContextDecoder, productDecoder, cartDecoder, positionDecoder, changeProductQuantityDecoder, orderCartDecoder, orderCartResonseDecoder, stockDecoder


# Encoder

@docs toCheckoutContextEncoder, toProductEncoder, toCartEncoder, toPositionEncoder, toChangeProductQuantityEncoder, toOrderCartEncoder, toOrderCartResonseEncoder, toStockEncoder
...
type CheckoutContextMsg
    = CheckoutContextMsgChangeProductQuantity ChangeProductQuantity
    | CheckoutContextMsgStock Stock
    | CheckoutContextMsgProduct Product
    | CheckoutContextMsgCartOrder OrderCart
...
checkoutContextDecoder =
    Decode.message (CheckoutContext Nothing)
        [ Decode.oneOf
            [ ( 1, Decode.map CheckoutContextMsgChangeProductQuantity changeProductQuantityDecoder )
            , ( 2, Decode.map CheckoutContextMsgStock stockDecoder )
            , ( 3, Decode.map CheckoutContextMsgProduct productDecoder )
            , ( 4, Decode.map CheckoutContextMsgCartOrder orderCartDecoder )
            ]
